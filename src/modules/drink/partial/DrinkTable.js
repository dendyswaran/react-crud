...
export default function DrinkTable() {
    ...
    const { datatable } = useSelector(state => state.drinkReducer)
    ...
    const handleFetchDatatable = (_filters) => {
        dispatch(genFetchDatatable(
            `/api/drinks/datatable`,
            {
                tableParams,
                dtSearch: _filters ? _filters : null
            },
            DRINK_ACTION.DRINK_DATATABLE_SUCCESS))
    }
    ...
    const handleRowClicked = (opt) => {
        navigate(`/drink/edit/${opt.data.id}`)
    }
